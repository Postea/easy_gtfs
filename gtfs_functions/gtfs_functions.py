...
def save_gdf(data, file_name, geojson=False, shapefile=True):
    ...
    # -------------------------------------------------------
    if geojson:
        data.to_file(filename=geojson_path, driver="GeoJSON")
    ...
    if shapefile:
        ...
        for ex in extensions:
            ...
            os.remove(
                file_name + ex
            )  # in case I want to remove the files out of the shapefile
...
def cut_gtfs(stop_times, stops, shapes):
    ...
    sseq = stop_times.drop_duplicates(
        subset=['stop_id', 'stop_name', 'stop_sequence', 'shape_id']
    )[['route_id', 'direction_id', 'stop_id', 'stop_name', 'stop_sequence', 'shape_id']]

    # Data frames with the number of stops for each route and direction and shape_id
    route_shapes = sseq.pivot_table(
        'stop_id', index=['route_id', 'direction_id', 'shape_id'], aggfunc='count'
    ).reset_index()
    ...
    def find_shape_closest_points(shape_id):
        ...
        route_id = route_shapes.loc[
            route_shapes.shape_id == shape_id, 'route_id'
        ].values[0]
        direction_id = route_shapes.loc[
            route_shapes.shape_id == shape_id, 'direction_id'
        ].values[0]
        ...
        route_stop_ids = sseq.loc[
            (sseq['route_id'] == route_id)
    ...
    def create_line(row):
        ...
        if row[0] == row[1]:
            ...
            if float(x2) != float(x1):
                ...
                if (
                    x2 - x1 < 0
                ):  # We should create an "if" to check if we need to do -1 or +1 depending on x2-x1
    ...
    shape_trans_lines = [
        find_shape_trans_lines(shape_closest_points[i])
        for i in range(0, len(shape_closest_points))
    ]
    ...
    def cut_shapes_(shape_trans_lines, shape_closest_points):
        ...
        if line.is_simple:
            ...
            d = dict(
                ...
                stop_id=list(df.stop_id)[:-1],
                stop_sequence=list(df.stop_sequence)[:-1],
            )
    ...
    segments = [
        cut_shapes_(shape_trans_lines[i], shape_closest_points[i])
        for i in range(0, len(shape_trans_lines))
    ]
    ...
    shapes_no_middle = shapes.loc[
        shapes.shape_id.isin(
            trans_loop.loc[trans_loop['n_segments'] == 3, 'shape_id'].unique()
        )
    ].reset_index()
    ...
    shapes_one_seg = shapes.loc[
        shapes.shape_id.isin(
            trans_loop.loc[trans_loop['n_segments'] == 2, 'shape_id'].unique()
        )
    ].reset_index()
    ...
    def add_points(row, add_p, cut_points_gdf):
        ...
        p = cut_points_gdf.loc[
            cut_points_gdf.index.isin(
                add_p.loc[add_p.index_track_ == index_track_, 'index_cut']
            )
        ]
        p.crs = {'init': 'epsg:4326'}

        seg = [
            LineString([p.geometry.values[i], p.geometry.values[i + 1]])
        ...
        new_points = (
            [start]
            + [
                gse.values[0].interpolate(dist * n)
                for n in list(range(1, num_vert + 1))
            ]
            + [end]
        )
    ...
    def cut_loops_shapes_ok(shape_id):
        ...
        cut_lines = trans_loop.loc[
            trans_loop.shape_id == shape_id, 'trans_lines'
        ].values[0][1:-1]
        cut_points = [
            x[1]
            for x in cut_points_loop.loc[
                cut_points_loop.shape_id == shape_id, 'points'
            ].values[0][1:-1]
        ]

        cut_gdf = gpd.GeoDataFrame(data=list(range(len(cut_lines))), geometry=cut_lines)
        cut_points_gdf = gpd.GeoDataFrame(
            data=list(range(len(cut_points))), geometry=cut_points
        )
        ...
        track_l = gpd.GeoSeries(
            [LineString([shape[i], shape[i + 1]]) for i in range(0, len(shape) - 1)]
        )
        ...
        track_l_gdf = gpd.GeoDataFrame(
            data=dict(distance_m=track_dist), geometry=track_l
        )
        ...
        how_many.rename(
            columns=dict(index='index_track_', index_right='index_cut'), inplace=True
        )

        # The filter those that were intersected by more than one cut line
        how_manyp = how_many.pivot_table(
            'geometry', index='index_track_', aggfunc='count'
        ).reset_index()
        how_manyp = how_manyp.loc[how_manyp.geometry > 1]

        add_p = how_many.loc[
            how_many.index_track_.isin(how_manyp.index_track_.unique())
        ]

        # Add intermediate points for segments with length > 100m
        track_l_gdf.loc[
            track_l_gdf.index.isin(how_manyp.index_track_.unique()), 'geometry'
        ] = track_l_gdf.loc[
            track_l_gdf.index.isin(how_manyp.index_track_.unique())
        ].apply(
            lambda x: add_points(x, add_p, cut_points_gdf), axis=1
        )
        ...
        flat_list = [item for sublist in t for item in sublist] + [
            track_l_gdf.geometry.tail(1).values[0].coords[-1]
        ]
        ...
        for l in range(1, len(cut_lines)):
            nearest_point = list(
                cut_points[l - 1].coords
            )  # segments always start in the one of the cut points
            start_iterator = (
                last_point + 1
            )  # start from the last point found in the previous segment

            for i in range(start_iterator, len(line.coords) + 1):
                points_to_stop = (
                    nearest_point + line.coords[last_point:i]
                )  # keep adding points to extend the line
                segment = LineString(points_to_stop)
                ...
                if segment.intersects(cut_lines[l]):
                    ...
                    points_to_stop = (
                        nearest_point
                        + line.coords[last_point : i - 1]
                        + list(cut_points[l].coords)
                    )
                    segment = LineString(points_to_stop)
    ...
    if run_shapes_no_middle:
        ...
        def cut_shapes_no_middle(shape_id):
            ...
            line = shapes_no_middle.loc[
                shapes_no_middle.shape_id == shape_id, 'geometry'
            ].values[0]
            cut_lines = trans_loop.loc[
                trans_loop.shape_id == shape_id, 'trans_lines'
            ].values[0][1:-1]
            ...
            points_to_stop = (
                nearest_point + line.coords[last_point - 1 : len(line.coords)]
            )
            segment = LineString(points_to_stop)
        ...
        # Apply the function
        segments2 = [
            cut_shapes_no_middle(s) for s in shapes_no_middle.shape_id.unique()
        ]
        # Remove None values
    ...
    if run_shapes_one_seg:
        ...
        def cut_shapes_one_seg(shape_id):
            ...
            line = shapes_one_seg.loc[
                shapes_one_seg.shape_id == shape_id, 'geometry'
            ].values[0]
            d['segment'] = [line]
            return d
    ...
    df = pd.concat(
        [
            format_shapes(
                s, sseq.loc[sseq.shape_id == s['shape_id']].tail(1).stop_id.values[0]
            )
            for s in segments
        ]
    )
...
def speeds_from_gtfs(
    ...
    speeds = pd.merge(
        ...
        how='left',
    ).drop('start_stop_id', axis=1)

    speeds = speeds.loc[
        ~speeds.distance_m.isnull(),
        [
            'trip_id',
            'route_id',
            'direction_id',
    ...
    if max(cutoffs) <= 24:
        ...
        speeds_fix['departure_time'] = [
            d - 24 * 3600 for d in speeds_fix.departure_time
        ]
    ...
    speeds['window'] = pd.cut(
        speeds['departure_time'], bins=cutoffs, right=False, labels=labels
    )
    speeds = speeds.loc[~speeds.window.isnull()]
    speeds['window'] = speeds['window'].astype(str)

    # Calculate the speed
    speeds.loc[speeds.runtime_h == 0.0, 'runtime_h'] = speeds.loc[
        speeds.runtime_h != 0.0, 'runtime_h'
    ].mean()
    ...
    avg_speed_route = speeds.pivot_table(
        'speed', index=['route_id', 'direction_id', 'window'], aggfunc='mean'
    ).reset_index()
    ...
    speeds.loc[speeds.speed > 120, 'speed'] = speeds.loc[
        speeds.speed > 120, 'avg_speed_route'
    ]

    # Calculate max speed per segment to have a free_flow reference
    max_speed_segment = speeds.pivot_table(
        'speed', index=['stop_id', 'direction_id'], aggfunc='max'
    )
    ...
    data_complete1 = data_complete.loc[
        ~data_complete.route_name.isnull(), :
    ].reset_index()
    ...
    data_complete1 = data_complete1.loc[
        :,
        [
            'route_id',
            'route_name',
    ...
    data_complete1 = pd.merge(
        data_complete1,
        max_speed_segment,
        left_on=['s_st_id', 'dir_id'],
        right_on=['stop_id', 'direction_id'],
        how='left',
    )

    gdf = gpd.GeoDataFrame(
        data=data_complete1.drop('geometry', axis=1), geometry=data_complete1.geometry
    )
...
def create_json(
    ...
    if symbol_layer:
        ...
        if colors == []:
            ...
            fill_color = pd.DataFrame(
                dict(
                    variable=gdf[variable].unique(),
                    fill_color=colors[0 : len(gdf[variable].unique())],
                )
            )
            gdf = pd.merge(
                gdf, fill_color, left_on=variable, right_on='variable', how='left'
            )

        d = dict(category=categories, symbol=symbols)

        category_symbols = pd.DataFrame(d)

        gdf = pd.merge(gdf, category_symbols, how='left')

        var_symbol_color = gdf.pivot_table(
            'id', index=[variable, 'symbol', 'fill_color'], aggfunc='count'
        ).reset_index()
        var_symbol_color['symbol_color'] = var_symbol_color.apply(
            lambda x: '{}{}'.format(x.symbol, x.fill_color), axis=1
        )
        ...
        for v in gdf.variable.unique():
            aux = dict(
                input=v,
                value=var_symbol_color.loc[
                    var_symbol_color[variable] == v, 'symbol_color'
                ].values[0],
            )
            symbols = symbols + [aux]
        ...
        label = dict(type='data-column', dataCol='name')

        t = dict(type='symbol', icon=icon, label=label, configVersion=1)
    else:
        # All line and circle numerical variable layers thing
        if colors == []:
            colors = (
                ["#D83D25", "#EF6933", "#F89041", "#fee090", "#91bfdb", "#4575b4"],
            )
        ...
        color = dict(
            type='range', stops=stops_color, dataCol=variable, maxInput=max_value
        )
        ...
        if gdf.geom_type[0] == 'Point':
            radius = dict(
                type='range', stops=stops_size, dataCol=variable, maxInput=max_value
            )
            gtype = 'circle'
            ...
            print(
                "Check the geometry, it is not recognized as a LineString nor a Point"
            )
        ...
        legendLabels = dict(
            dataColLabels={
                filter_variables1[i]: filter_labels1[i]
                for i in range(len(filter_variables1))
            }
        )
        ...
        for f in filter_variables:
            if (f == 'route_name') & ('All lines' in list(gdf[f].unique())):
                aux = dict(
                    values=['All lines']
                    + list(
                        gdf.loc[gdf.route_id != 'ALL_LINES']
                        .route_name.sort_values(ascending=True)
                        .unique()
                    ),
                    dataCol='route_name',
                ...
                if default_values[filter_variables.index(f)] == True:
                    aux = dict(
                        values=[
                            str(x) for x in gdf[f].sort_values(ascending=True).unique()
                        ],
                        dataCol=f,
                        defaultValue=str(
                            list(gdf[f].sort_values(ascending=True).unique())[0]
                        ),
                    )
                else:
                    aux = dict(
...
def stops_freq(stop_times, stops, cutoffs=[0, 6, 9, 15, 19, 22, 24]):
    ...
    # Some gtfs feeds only contain direction_id 0, use that as default
    stop_times['direction_id'] = stop_times['direction_id'].fillna(0)
    trips_per_window = stop_times.pivot_table(
        'trip_id', index=['stop_id', 'direction_id', 'window'], aggfunc='count'
    ).reset_index()
    trips_per_hour = stop_times.pivot_table(
        'trip_id', index=['stop_id', 'direction_id', 'hour'], aggfunc='count'
    ).reset_index()
    ...
    max_trips = trips_per_hour.pivot_table(
        'max_trips', index=['stop_id', 'direction_id'], aggfunc='max'
    ).reset_index()
    max_freq = trips_per_hour.pivot_table(
        'max_frequency', index=['stop_id', 'direction_id'], aggfunc='min'
    ).reset_index()
    ...
    end_time = trips_per_window['window'].apply(
        lambda x: int(re.search('-(.*?):', x).group(1))
    )

    trips_per_window['frequency'] = (
        (end_time - start_time) * 60 / trips_per_window.ntrips
    ).astype(int)
    ...
    stop_frequencies = pd.merge(
        stop_frequencies, stops.loc[:, ['stop_id', 'stop_name', 'geometry']], how='left'
    )
    ...
    stop_frequencies.loc[
        stop_frequencies.direction_id == 1, 'direction_id'
    ] = 'Outbound'

    stop_frequencies.rename(
        columns={'direction_id': 'dir_id', 'max_frequency': 'max_freq'}, inplace=True
    )
...
def map_gdf(
    ...
    m = folium.Map(
        location=[centroid_lat, centroid_lon], tiles='cartodbpositron', zoom_start=12
    )
    ...
    if gdf.geom_type.values[0] == 'Point':
        ...
        folium.GeoJson(
            ...
            tooltip=folium.features.GeoJsonTooltip(
                fields=tooltip_var, aliases=tooltip_labels, labels=True, sticky=False
            ),
        ).add_to(m)
...
def lines_freq(stop_times, trips, shapes, routes, cutoffs=[0, 6, 9, 15, 19, 22, 24]):
    ...
    trips_per_window = stop_times_first.pivot_table(
        'trip_id', index=['route_id', 'direction_id', 'window'], aggfunc='count'
    ).reset_index()
    trips_per_hour = stop_times_first.pivot_table(
        'trip_id', index=['route_id', 'direction_id', 'hour'], aggfunc='count'
    ).reset_index()
    ...
    max_trips = trips_per_hour.pivot_table(
        'max_trips', index=['route_id', 'direction_id'], aggfunc='max'
    ).reset_index()
    max_freq = trips_per_hour.pivot_table(
        'max_frequency', index=['route_id', 'direction_id'], aggfunc='min'
    ).reset_index()
    ...
    aux = trips.loc[
        trips.service_id == '1', ['route_id', 'direction_id', 'shape_id']
    ].drop_duplicates()
    ...
    line_frequencies_gdf = pd.merge(
        line_frequencies_gdf, routes[['route_id', 'route_name']]
    )

    gdf = gpd.GeoDataFrame(
        data=line_frequencies_gdf.drop('geometry', axis=1),
        geometry=line_frequencies_gdf.geometry,
    )
...
def segments_freq(segments_gdf, stop_times, routes, cutoffs=[0, 6, 9, 15, 19, 22, 24]):
    ...
    trips_per_window = stop_times.pivot_table(
        'trip_id',
        index=['route_id', 'stop_id', 'direction_id', 'window'],
        aggfunc='count',
    ).reset_index()
    trips_per_hour = stop_times.pivot_table(
        'trip_id',
        index=['route_id', 'stop_id', 'direction_id', 'hour'],
        aggfunc='count',
    ).reset_index()
    ...
    # Get max number of trips and highest frequency
    max_trips = trips_per_hour.pivot_table(
        'max_trips', index=['route_id', 'stop_id', 'direction_id'], aggfunc='max'
    ).reset_index()
    max_freq = trips_per_hour.pivot_table(
        'max_frequency', index=['route_id', 'stop_id', 'direction_id'], aggfunc='min'
    ).reset_index()

    # Calculate frequency per window for each route
    trips_per_window.rename(columns={'trip_id': 'ntrips'}, inplace=True)
    start_time = trips_per_window['window'].apply(
        lambda x: int(x.split(':')[0]) + (int(x.split(':')[1][:2]) / 60)
    )
    end_time = trips_per_window['window'].apply(
        lambda x: int(re.search('-(.*?):', x).group(1)) + (int(x.split(':')[2]) / 60)
    )
    ...
    line_frequencies.drop_duplicates(
        subset=[
            'route_id',
            'stop_id',
            'direction_id',
    ...
    line_frequencies = pd.merge(
        line_frequencies, routes.loc[:, ['route_id', 'route_name']], how='left'
    )

    # Calculate sum of trips per segment with all lines
    all_lines = line_frequencies.pivot_table(
        ['ntrips'], index=['segment_id', 'window'], aggfunc='sum'
    ).reset_index()

    # Calculate frequency per window for all routes
    start_time = all_lines['window'].apply(
        lambda x: int(x.split(':')[0]) + (int(x.split(':')[1][:2]) / 60)
    )
    end_time = all_lines['window'].apply(
        lambda x: int(re.search('-(.*?):', x).group(1)) + (int(x.split(':')[2]) / 60)
    )

    all_lines['frequency'] = ((end_time - start_time) * 60 / all_lines.ntrips).astype(
        int
    )

    # Get max number of trips and highest frequency per segment for all routes
    max_trips_all_lines = all_lines.pivot_table(
        'ntrips', index=['segment_id'], aggfunc='max'
    ).reset_index()
    max_freq_all_lines = all_lines.pivot_table(
        'frequency', index=['segment_id'], aggfunc='min'
    ).reset_index()
    ...
    gdf = gpd.GeoDataFrame(
        data=data_complete.drop('geometry', axis=1), geometry=data_complete.geometry
    )
